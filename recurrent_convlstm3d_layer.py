...
from tensorflow.keras.layers import Input,ConvLSTM3D
...
if __name__ == "__main__":
    ...
    outputs = ConvLSTM3D(2,3,activation=None,data_format='channels_last')(inputs)
    ...
    writer = cv2.VideoWriter('result_recurrent_convlstm3d_filter_1.mp4', fmt, fps, (filter_1_width, filter_1_height))
    for n in range(filter_1_frames):
        bgr = cv2.cvtColor(filter_1[n], cv2.COLOR_GRAY2BGR)
        writer.write(bgr)
    writer.release()
    
    writer = cv2.VideoWriter('result_recurrent_convlstm3d_filter_2.mp4', fmt, fps, (filter_2_width, filter_2_height))
